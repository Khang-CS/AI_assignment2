...
def main(mode = 0, firstTurn = True):
    ...
    while running:
        # human is white, go first
        if mode == 1 and not gameOver:
            # check if human turn (white)
            ...
            if playerTurn:
                ...
                if Minimax_move is None:
                    Minimax_move = MoveDecision.makeRandomMove(validMoves)
                if Minimax_move.isPromote:
                    Minimax_move.promoteTo = MoveDecision.selectRandomPromote('q')
            ...
            # Random Agent move
                ...
                for e in p.event.get():
                    if e.type == p.QUIT:
                        running = False
                RandomMove = MoveDecision.makeRandomMove(validMoves)
                if RandomMove.isPromote:
                    RandomMove.promoteTo = MoveDecision.selectRandomPromote()
                
                gs.makeMove(RandomMove)
                print(RandomMove.getChessNotation())
                moveMade = True                
